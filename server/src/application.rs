...
use app_ops::{utils::HttpSettings, AppOpsExt, GetAppInfoResponseBuild, HttpAppRootSpanBuilder};
use tokio::task::JoinHandle;

use crate::application::StartUpError::{
    FailToStartHttpServer, FailToStartTcpListener, FailToStartUpBrokerConnection,
};
use crate::routes::*;
...
impl Application {
    ...
    pub async fn start(
    ...
    ) -> Result<(Server, JoinHandle<tokio::io::Result<()>>), StartUpError> {
        ...
        let endpoint = app_settings
            ...
            .collect::<Vec<&str>>();
        let lb_channel =
            match LoadBalancedChannel::builder((endpoint[0], endpoint[1].parse::<u16>().unwrap()))
                .await
            {
                Ok(v) => v.channel(),
                Err(e) => {
                    error!(
                        "fail to initialise dns resolver for connection to broker {:?}",
                        e
                    );
        ...
        let server = HttpServer::new(move || {
            App::new()
                ...
                .wrap(create_cors_policy(
                    &app_settings.settings.http.allowed_origin,
                ))
        ...
        .listen(listener)
        .map_err(|e| FailToStartHttpServer(e))?;
